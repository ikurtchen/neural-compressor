...
class PatchedModuleFusedSDPA(PatchedModuleBase):
    ...
    def forward_quant(
        ...
        if self.qkv_slice_thld > 0 and q_len != 1 and q_len != kv_len and kv_len > self.qkv_slice_thld:
            ...
            prefix_key = kinput[..., 0:ctx_len, :]
            prefix_value = vinput[..., 0:ctx_len, :]

            # calculate the first prefix sdpa w/o mask
            prefix_results = self.fp8_fsdpa_fwd(qinput, prefix_key, prefix_value, None, dropout_p, scale, False, sm_mode)
